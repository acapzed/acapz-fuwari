...
보통 이런 프로젝트에서 사용되는 언어는 C++이다. 다만 전에 sdl2 + mingw 빌드환경 만들면서 쓸데없는 개고생을 한적이 있어서 뭔가 거부감이 들기도 했고, C++는 메모리 관리하기가 되게 힘들다는 악명이 돌기도 했다. 결정적으로 C++에 그렇게 익숙하지 않아서 애초에 처음 배울거면 최근 유행하는 go나 rust와 같은 언어를 해보는것도 나쁘지 않겠다 싶어서 무턱대고 C++를 고르지 않았다.